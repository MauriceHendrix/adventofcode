...
def part2c(puzzle_input: list[str]):
    ...
    while card_num < len(puzzle_input):
        count = 0
        exists = {}
        for num in cards[card_num-1][0]:
            exists[num]=0
        for num in cards[card_num-1][1]:
            exists[num] = exists.get(num,-1)+1
        count = sum(exists.values())
...
def part2d(puzzle_input: list[str]):
    ...
    while card_num < len(puzzle_input):
        ...
        exists = {}
        i = 0
        while i < len(cards[card_num-1][0]):
            j = 0
            while j < len(cards[card_num-1][1]) and i < len(cards[card_num-1][0]):
                if(cards[card_num-1][0][i] == cards[card_num-1][1][j]):
                    count += 1
                    i, j = i+1, j+1
                elif(cards[card_num-1][0][i] < cards[card_num-1][1][j]):
                    i += 1
                elif(cards[card_num-1][0][i] > cards[card_num-1][1][j]):
                    j += 1
            i += 1
...
@timing
def part2e(puzzle_input: list[str]):
    cards = []
    cards_dup = []
    for card in puzzle_input:
        score = 0
